  ...
  onWindowScroll() {
    if (!this.isScrolled && !this.isNoRecords) {
      ...
      // this.windowBottom=(windowHeight + window.pageYOffset)-880;
       ...
       if(!this.isEditorForEditThePublish && !this.isEditorForEditTheDraft && !this.isEditorForEditTheOnline && !this.isBlogDraft && !this.isIonizedBlog && !this.isPublishedBlog)
             {
...
// publish Calendar
blogCalendarCall() {
  this.router.navigate(['publish/calendar']);
...
static calenderOverEvent(view) {
  ...
  console.log(PublishComponent.viewData)
  if(view.state==3)
    {
...
addNewTrendingTopic() {
  ...
  const resData = 
  {
    ...
    console.log(resData);
    this.publishService.createTrendingTopic(resData).subscribe(
